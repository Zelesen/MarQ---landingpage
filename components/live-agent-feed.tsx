...
const AGENT_NAMES = [
  "llama-3.1-8b", "mistral-7b-v0.3", "phi-3.5-mini", "gemma-2-9b",
  "qwen-2.5-7b", "flux-1-schnell", "whisper-v3", "moondream-2",
  "stable-diffusion-3", "command-r-v1",
]

const TASKS = [
  "Calculating perplexity scores",
  "Verifying SHA-256 integrity",
  "Generating GGUF shards for R2",
  "Profiling latency on Metal M3",
  "Converting safetensors to ONNX",
  "Validating AWQ 4-bit weights",
  "Optimizing for TFLite Mobile",
  "Benchmarking FID scores",
  "Sharding weights for CDN",
  "Updating metadata API manifest",
  "Compressing shards for edge",
  "Validating logic on H100",
  "Encrypting verified weights",
  "Syncing CDN global caches",
]

const REGIONS = ["GGUF", "GPTQ", "AWQ", "ONNX", "TFLite"]
const STATUSES = [
  { label: "available",  color: "#4ade80" },
  { label: "available",  color: "#4ade80" },
  { label: "available",  color: "#4ade80" },
  { label: "processing", color: "#facc15" },
  { label: "verified",   color: "#60a5fa" },
]
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {["MODEL", "ACTION", "FORMAT", "STATUS"].map(h => (
