...
const ALL_PRS = [
  { id: 145, title: "quant: llama-3.1-70b-gguf",               agent: "worker-h100",    status: "review",  comments: 2,  additions: 57,  deletions: 4,  branch: "r2/weights",           time: "Just now" },
  { id: 144, title: "quant: mistral-nemo-12b-awq",             agent: "worker-a10g",    status: "review",  comments: 1,  additions: 18,  deletions: 3,  branch: "r2/weights",           time: "1m ago" },
  { id: 143, title: "quant: phi-3.5-mini-onnx",                agent: "worker-h100",    status: "merged",  comments: 4,  additions: 93,  deletions: 11, branch: "r2/weights",           time: "1m ago" },
  { id: 142, title: "quant: gemma-2-9b-it-gptq",               agent: "worker-a10g",    status: "merged",  comments: 3,  additions: 84,  deletions: 12, branch: "r2/weights",           time: "2m ago" },
  { id: 141, title: "quant: qwen-2.5-7b-gguf",                 agent: "worker-h100",    status: "approved",comments: 1,  additions: 31,  deletions: 8,  branch: "r2/weights",           time: "8m ago" },
  { id: 140, title: "quant: flux-1-schnell-fp8",               agent: "worker-h100",    status: "review",  comments: 5,  additions: 142, deletions: 27, branch: "r2/weights",           time: "22m ago" },
  { id: 139, title: "quant: whisper-large-v3-tflite",          agent: "worker-a10g",    status: "merged",  comments: 7,  additions: 209, deletions: 88, branch: "r2/weights",           time: "1h ago" },
]

const ALL_REVIEW_FILES = [
  { file: "weights/shard_01.bin",    pct: 72 },
  { file: "weights/shard_02.bin",    pct: 45 },
  { file: "metadata/config.json",    pct: 88 },
  { file: "benchmarks/latency.csv",  pct: 31 },
  { file: "weights/manifest.sha",    pct: 60 },
]

const ALL_REVIEW_LINES: { type: "comment"|"approve"|"change"|"code"; text: string; author?: string }[] = [
  { type: "code",    text: 'Verifying weights integrity...' },
  { type: "comment", text: "SHA-256 Checksum verified.", author: "validator-node" },
  { type: "code",    text: 'Calculating perplexity scores...' },
  { type: "approve", text: "LGTM — Quantization loss < 0.02", author: "eval-worker" },
  { type: "code",    text: 'Running hardware latency profiling...' },
  { type: "change",  text: "High VRAM usage on T4 reference", author: "hardware-bot" },
  { type: "code",    text: 'Optimizing GGUF shards for R2...' },
  { type: "approve", text: "Verified — Ready for registry", author: "registry-main" },
  { type: "code",    text: 'Updating global metadata API...' },
  { type: "comment", text: "Added tag: verified-hardware", author: "metadata-svc" },
  { type: "code",    text: 'Pushing to edge distribution...' },
  { type: "approve", text: "All shards available globally", author: "cdn-node" },
]

const COMMITS = [
  { hash: "a3f8c21", msg: "update: llama-3.1-8b (new q8_0 variant)",    time: "Just now" },
  { hash: "b7d2e09", msg: "feat: added onnx support for phi-3-mini",     time: "4m ago"   },
  { hash: "c9a1f34", msg: "chore: bump weights schema to v2.4.1",        time: "12m ago"  },
  { hash: "d4e6b78", msg: "perf: reduce egress latency by 18%",          time: "31m ago"  },
  { hash: "e2c9d56", msg: "feat: validated coreml for moondream",        time: "1h ago"   },
]
...
function StatusBadge({ status }: { status: string }) {
  const cfg = {
    merged:   { bg: "rgba(130,80,255,0.1)",  color: "#8250df", icon: <GitMerge style={{ width: 9, height: 9 }} />,  label: "Verified" },
    approved: { bg: "rgba(40,167,69,0.1)",   color: "#28a745", icon: <CheckCircle2 style={{ width: 9, height: 9 }} />, label: "Ready"    },
    review:   { bg: "rgba(201,169,110,0.12)",color: "#b07d30", icon: <Eye style={{ width: 9, height: 9 }} />,       label: "Processing"},
    ...
    >
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(251,250,247,0.9)" }}>
          {[
            { label: "Models Quantized",  val: 18,       icon: <GitMerge style={{ width: 11, height: 11 }} />,   graph: <MiniBarGraph seed={0} /> },
            { label: "Weights Verified",  val: 34,       icon: <Eye style={{ width: 11, height: 11 }} />,         graph: <MiniBarGraph seed={5} /> },
            { label: "Registry updates",  val: 127,      icon: <GitCommit style={{ width: 11, height: 11 }} />,   graph: <MiniDotGraph seed={2} /> },
            { label: "Egress / sec",      val: reqCount, icon: <Zap style={{ width: 11, height: 11 }} />,         graph: <LiveSparkline seed={7} /> },
          ].map((m, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 0.85fr", gap: 8, padding: 8, height: 340, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(160) }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                ...
                <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Download Registry</span>
              </div>
              <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.25)", fontFamily: "monospace" }}>{ALL_PRS.filter(p => p.status === "review").length} PENDING</span>
            </div>
            ...
            <div style={{ ...panel, padding: "8px 10px", flexShrink: 0, height: 76, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 7.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.33)", fontFamily: "monospace" }}>Registry Activity</span>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(210) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Weight Validation — #{ALL_PRS[reviewFileIdx % ALL_PRS.length].id}</span>
            </div>
            <div style={{ ...panel, flex: 1, padding: "9px 10px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ marginBottom: 7, paddingBottom: 7, borderBottom: "1px solid rgba(0,0,0,0.05)", flexShrink: 0 }}>
                <div style={{ fontSize: 9.5, fontWeight: 600, color: "#111", marginBottom: 2 }}>quant: flux-1-schnell-fp8</div>
                <div style={{ display: "flex", gap: 5 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(260) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Registry Updates</span>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "2px 2px 0", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Verification Pipeline</span>
            </div>
            <div style={{ ...panel, flexShrink: 0, overflow: "hidden" }}>
              {[
                { name: "gguf-quantizer",   status: "passing", duration: "1m 32s" },
                { name: "perplexity-eval",  status: "running", duration: "0m 48s" },
                { name: "shard-validator",  status: "passing", duration: "2m 11s" },
                { name: "cdn-optimizer",    status: "running", duration: "0m 54s" },
              ].map((a, i) => (
